<template>
  <div class="col-md-8">
    <h6 class="text-muted text-normal text-uppercase ">Followings</h6>
    <hr class="mb-3 mt-2">
    <div class="card-new-layout" v-if="loaded&&user_followings.length">
      <div class="team team-grid">
        <div class="row">
          <div class="col-md-6 mb-4" v-for="following in user_followings">
            <div class="d-table">
              ...
              <div class="pl-3 d-table-cell align-middle">
                ...
                <span class="text-muted text-sm mb-2">{{following.title}}</span> <span v-if="following.company" class="text-gray-dark">@ {{following.company.name}}</span>
    ...
    <div class="card-new-layout" v-else-if="loaded&&!user_followings.length">
      <h6 class="mb-0 text-center">{{user_info.full_name}} is not following anyone</h6>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      getFollowings () {
        this.loaded = false
        this.$store.dispatch('getUserFollowings', this.$route.params.id)
          .then(() => {
            this.loaded = true
          })
      }
